# Batch_076 Verification Pipeline
...
$suspiciousDir = "D:\数据收集\luotianyi-db\processed\rag\suspicious_music\Batch_076"
...
$files = $assignments.Batch_076 | ForEach-Object { $_.value }
Write-Host "Processing Batch_076: $($files.Count) files"
...
foreach ($file in $files) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($videoIdRaw) -and $videoIdRaw -notmatch '参见|待确认|待补充|未获取|未确认|未找到|待查|推测|--') {
    ...
    if ([string]::IsNullOrEmpty($primaryBV)) {
        # Check source URLs for BVs
        $bilibiliUrls = [regex]::Matches($content, 'https?://(?:www\.)?bilibili\.com/video/[^\s\)\]>"]+')
        foreach ($u in $bilibiliUrls) {
            $bvMatch = [regex]::Match($u.Value, 'BV[a-zA-Z0-9]{10,12}')
            if ($bvMatch.Success -and $bvMatch.Value.Length -ge 11) {
                $primaryBV = $bvMatch.Value; $foundInSource = $true; break
            }
        }
    }
    if ([string]::IsNullOrEmpty($primaryBV)) {
    ...
    if ($foundInSource -and -not [string]::IsNullOrEmpty($primaryBV)) {
        try {
            ...
            if ($response.code -eq 0) {
                ...
                if (-not [string]::IsNullOrEmpty($pzhu)) {
                    ...
                    # Also strip parenthetical aliases
                    $pClean = $pClean -replace '\(.*?\)', ''
                    $pCleanLower = $pClean.ToLower().Trim()
                    $apiOwnerLower = $apiOwner.ToLower().Trim()
        ...
        if ([string]::IsNullOrWhiteSpace($videoIdRaw)) {
...
$meta = @{
    ...
    range = "xianrenwuyu.md ~ xiaoyifanzi-lezhengling.md"
...
$reportPath = Join-Path -Path $reportDir -ChildPath "Batch_076_verification.json"
...
Write-Host "`n=== BATCH 076 VERIFICATION SUMMARY ==="
...
$entries | Select-Object filename, classification, primary_BV, @{N='曲名(25)';E={$_.song_name.Substring(0,[Math]::Min(25, $_.song_name.Length))}}, @{N='Notes(65)';E={$_.notes.Substring(0,[Math]::Min(65, $_.notes.Length))}} | Format-Table -AutoSize
