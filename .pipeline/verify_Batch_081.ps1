# Batch_081 Verification Pipeline
...
$suspiciousDir = "D:\数据收集\luotianyi-db\processed\rag\suspicious_music\Batch_081"
...
$files = $assignments.Batch_081 | ForEach-Object { $_.value }
Write-Host "Processing Batch_081: $($files.Count) files"
...
foreach ($file in $files) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($videoIdRaw) -and $videoIdRaw -notmatch '参见|待确认|待补充|未找到') {
    ...
    if ($foundInSource -and -not [string]::IsNullOrEmpty($primaryBV)) {
        try {
            ...
            if ($response.code -eq 0) {
                ...
                # Determine if song/API are completely different
...
$meta = @{
    ...
    range = "xuanwo.md ~ x-unknown-19.md"
...
$reportPath = Join-Path -Path $reportDir -ChildPath "Batch_081_verification.json"
...
Write-Host "`n=== BATCH 081 VERIFICATION SUMMARY ==="
