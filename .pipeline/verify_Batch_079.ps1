# Batch_079 Verification Pipeline
...
$suspiciousDir = "D:\数据收集\luotianyi-db\processed\rag\suspicious_music\Batch_079"
...
$files = $assignments.Batch_079 | ForEach-Object { $_.value }
Write-Host "Processing Batch_079: $($files.Count) files"
...
foreach ($file in $files) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($videoIdRaw) -and $videoIdRaw -notmatch '参见|待确认|待补充|未获取|未确认|未找到|待查|推测|暂未|--') {
...
$meta = @{
    ...
    range = "xinghui-long-ver.md ~ xinniantuixiu.md"
...
$reportPath = Join-Path -Path $reportDir -ChildPath "Batch_079_verification.json"
...
Write-Host "`n=== BATCH 079 VERIFICATION SUMMARY ==="
