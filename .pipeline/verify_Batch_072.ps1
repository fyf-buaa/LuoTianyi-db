# Batch_072 Verification Pipeline
...
$suspiciousDir = "D:\数据收集\luotianyi-db\processed\rag\suspicious_music\Batch_072"
...
$files = $assignments.Batch_072 | ForEach-Object { $_.value }
Write-Host "Processing Batch_072: $($files.Count) files"
...
foreach ($file in $files) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($videoIdRaw) -and $videoIdRaw -notmatch '参见|待确认|待补充') {
            ...
            if ($response.code -eq 0) {
                ...
                if (-not [string]::IsNullOrEmpty($pzhu)) {
                    ...
                    if ($apiOwnerLower.Contains($pCleanLower) -or $pCleanLower.Contains($apiOwnerLower)) {
                        ...
                        # Also check if the file content has the API owner name mentioned
                    $pMatch = $false
                    }
                }
                ...
                # Error 62002 = video not found/deleted
...
$meta = @{
    ...
    range = "woshidanshengou.md ~ wozhongjiumeinengzhujinnixinli.md"
...
$reportPath = Join-Path -Path $reportDir -ChildPath "Batch_072_verification.json"
...
Write-Host "`n=== BATCH 072 VERIFICATION SUMMARY ==="
